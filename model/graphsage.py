...
class GraphSAGE(nn.Module):
    ...
    def __init__(
        ...
        # input layer
        if self.norm == "batch":
            norm_layer = nn.BatchNorm1d(n_hidden)
        elif self.norm == "layer":
            norm_layer = nn.LayerNorm(n_hidden)
        else:
            norm_layer = None
        self.layers.append(
            SAGEConv(in_feats, n_hidden, aggregator_type, norm=norm_layer)
        )
        # hidden layers
        for i in range(n_layers - 1):
            if self.norm == "batch":
                ...
                norm_layer = None
            self.layers.append(
                SAGEConv(n_hidden, n_hidden, aggregator_type, norm=norm_layer)
            )
